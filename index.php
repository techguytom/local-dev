...
<html>
    <head>
        ...
        <title>Nerdery Development Sites</title>
    ...
    <body>
        <div class="canvas">
            <header>
                <h1>Nerdery Development Sites</h1>
                <nav>
                    <ul>
...
foreach ( $dir as $d ) {
    ...
    foreach( glob( $d ) as $file )  {
        ...
        $siteroot2 = sprintf( 'http://%1$s.%2$s', $project, $tld2 );

        // Display an icon for the site
        $icon_output = '<span class="no-img"></span>';
        foreach( $icons as $icon ) {

            if ( file_exists( $file . '/web/' . $icon ) ) {
        ...
        printf( '<a class="site" href="%1$s">%2$s</a>', $siteroot, $displayname . ' dev' );

        if (is_dir($file . '/build/output')) {
            printf( '|&nbsp;<a class="site" href="%1$s">%2$s</a>', $siteroot2, $displayname . ' prod' );
        }
